...
// 状态转移，一共有四种情况：
// 将 word1[i] 删除或在 word2[j] 后面添加 word1[i]，则其操作次数等于 f[i−1,j]+1； ===> 在word2[j]后面添加word1[i]之后word1[1...i]和word2[1...j]就匹配了，说明在添加之前word1[1...i-1]需要和word2[1...j]匹配，这一步的操作次数最小是f[i - 1, j] + 1
